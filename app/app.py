...
# SEND TO N8N
# ---------------------------
def send_to_n8n(candidate_name, rf_result, ann_score, match_score, status):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.warning(f"n8n error: {response.status_code}")
    except Exception as e:
        st.warning(f"n8n failed: {str(e)}")
    ...
    def predict(self, X):
        ...
        while i < len(self.weights):
            if self.weights[i].ndim == 2:
                dense_layers.append((self.weights[i], self.weights[i+1]))
                i += 2
            else:
                i += 1
...
<style>
    ...
    .hero-sub { color: #6b7280; font-size: 1rem; letter-spacing: 0.05em; text-transform: uppercase; }
    .glass-card {
    ...
    .metric-label { font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.1em; color: #6b7280; margin-bottom: 0.5rem; }
    .metric-value { font-family: 'Syne', sans-serif; font-size: 2.2rem; font-weight: 700; line-height: 1; }
    .metric-sub { font-size: 0.75rem; color: #6b7280; margin-top: 0.4rem; }
    ...
    .progress-label { display: flex; justify-content: space-between; font-size: 0.8rem; color: #9ca3af; margin-bottom: 0.3rem; }
    ...
    [data-testid="stSidebar"] { background: #0d1117 !important; border-right: 1px solid rgba(255,255,255,0.06); }
    .stButton > button {
...
def progress_bar(label, value_pct, fill_class):
    ...
    <div class="progress-wrap">
        <div class="progress-label"><span>{label}</span><span>{value_pct:.1f}%</span></div>
...
with st.sidebar:
    ...
    <div style="font-size:0.82rem; color:#9ca3af; line-height:1.8;">
        🟢 <b style="color:#34d399">≥ threshold</b> — Hire<br>
...
if run:
    if file and jd.strip():
        ...
        with c1:
            st.markdown(f'<div class="metric-card"><div class="metric-label">Match Score</div><div class="metric-value score-blue">{match_pct:.1f}%</div><div class="metric-sub">Cosine Similarity</div></div>', unsafe_allow_html=True)
        with c2:
            st.markdown(f'<div class="metric-card"><div class="metric-label">AI Score</div><div class="metric-value {score_class}">{ann_score:.1f}%</div><div class="metric-sub">ANN Confidence</div></div>', unsafe_allow_html=True)
        with c3:
            st.markdown(f'<div class="metric-card"><div class="metric-label">RF Score</div><div class="metric-value score-blue">{rf_proba:.1f}%</div><div class="metric-sub">Random Forest</div></div>', unsafe_allow_html=True)
        with c4:
            st.markdown(f'<div class="metric-card"><div class="metric-label">Decision</div><div style="margin-top:0.6rem"><span class="status-badge {badge_class}">{status}</span></div><div class="metric-sub">Threshold: {threshold}%</div></div>', unsafe_allow_html=True)
        ...
        with col_left:
            ...
            st.markdown(progress_bar("Cosine Match Score", match_pct, "fill-blue"), unsafe_allow_html=True)
            st.markdown(progress_bar("ANN AI Confidence", ann_score, fill_class), unsafe_allow_html=True)
            st.markdown(progress_bar("Random Forest Probability", rf_proba, "fill-blue"), unsafe_allow_html=True)
            st.markdown(progress_bar("Hire Threshold", threshold, "fill-yellow"), unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
